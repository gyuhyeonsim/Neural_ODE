...
class Sinusoid(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        freq = np.array(ast.literal_eval(freq))  # list representation of string -> list
        amp = data['amp'].to_numpy()[0]
        amp = np.array(ast.literal_eval(amp))
        mask = data['mask'].to_numpy()

        dict_to_return = {'t': t, 'point': point, 'freq': freq, 'amp': amp, 'mask':mask}
